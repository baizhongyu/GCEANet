...
# Basic options
parser.add_argument('--content_dir', type=str, default='.train/MS-COCO',
                    help='Directory path to a batch of content images')
parser.add_argument('--style_dir', type=str, default='./train/Wikiart',
...
# training options
parser.add_argument('--save_dir', default='./experiments/run',
                    help='Directory to save the model')
parser.add_argument('--log_dir', default='./logs1',
                    help='Directory to save the log')
parser.add_argument('--lr', type=float, default=1e-4)
parser.add_argument('--lr_decay', type=float, default=5e-5)
parser.add_argument('--max_iter', type=int, default=160000)#160000
...
network.train()
network = nn.DataParallel(network,device_ids=ids)
network.to(device)
#network.cuda()
...
def print_network_params(model,model_name):
    ...
    if isinstance(model,list):
        ...
        for param in model.parameters():
            num_params += param.numel()
        print('[Network %s] Total number of parameters : %.5f M' % (model_name, num_params / 1e6))
print_network_params(network,'GCNANet')
########################
...
for i in tqdm(range(args.start_iter, args.max_iter)):
    ...
    # content_images = torch.cat([content_images, content_images_], 0).to(device)
    #style_images = torch.cat([style_images, style_images], 0).to(device)
    ######################################################
    # print('content_size()',content_images.size())
    # print('style_size()', style_images.size())
    ...
    #img, loss_c, loss_s, loss_contrastive_c, loss_contrastive_s = network(content_images,
                                                                                                    # style_images,
                                                                                                    # args.batch_size)
    # train discriminator
    # loss_gan_d = D.module.compute_loss(style_images, valid) + D.module.compute_loss(img.detach(), fake)###rensor.detach()??????????
    ...
    #loss_gan_g = args.gan_weight * D.module.compute_loss(img, valid)
    ...
    # writer.add_scalar('loss_content', torch.tensor(loss_c).item(), i + 1)
    ...
    if (i + 1) % args.save_model_interval == 0 or (i + 1) == args.max_iter:
        ...
        torch.save(state_dict,
                   '{:s}/decoder_iter_{:d}.pth'.format(args.save_dir,
                                                           i + 1))
        state_dict = network.module.transform.state_dict()
        for key in state_dict.keys():
            state_dict[key] = state_dict[key].to(torch.device('cpu'))
        torch.save(state_dict,
